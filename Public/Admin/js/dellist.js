...
layui.define(['laypage', 'layer', 'form', 'pagesize'], function (exports) {
    ...
    function initilData(currentIndex, pageSize) {
        ...
        //模拟数据
        var data = new Array();
        $.get('http://www.blog.com/admin.php/Del/show',function(result){
            data = result;
        },'json');
        ...
        setTimeout(function () {
            ...
            html += '<thead><tr><th>发表时间</th><th>标题</th><th>作者</th><th>类别</th><th colspan="2">操作</th></tr></thead>';
            html += '<tbody>';
            //遍历文章集合
            for (var i = 0; i < data.length; i++) {
                ...
                html += '<td><button class="layui-btn layui-btn-small layui-btn-normal" onclick="layui.dellist.editData(\'' + item.title_id + '\')"><i class="layui-icon">&#xe642;</i></button></td>';
                html += '<td><button class="layui-btn layui-btn-small layui-btn-danger" onclick="layui.dellist.deleteData(\'' + item.title_id + '\')"><i class="layui-icon">&#xe640;</i></button></td>';
                html += "</tr>";
            }
            html += '</tbody>';
            html += '</table>';
    ...
    var dellist = {
        deleteData: function (id) {
            layer.confirm('确定永久删除？', {
                btn: ['确定', '取消'] //按钮
            }, function () {
                //layer.msg('删除Id为【' + id + '】的数据');
                var url='http://www.blog.com/admin.php/Del/del';
        ...
        editData: function (id) {
            ...
            layer.confirm('是否还原？', {
                ...
                //layer.msg('删除Id为【' + id + '】的数据');
                var url='http://www.blog.com/admin.php/Del/restore';
                $.get(url,{'id':id},function(msg){
                     if(msg==1){
                        location.href = location.href;
                     }
